async function equipBestToolOrArmor(bot, type) {
    ...
    if (type == "sword") {
        ...
        const Helmet =  bot.inventory.findInventoryItem(mcData.itemsByName.diamond_helmet.id)     ||
                        ...
                        bot.inventory.findInventoryItem(mcData.itemsByName.turtle_helmet.id)      ||
                        bot.inventory.findInventoryItem(mcData.itemsByName.carved_pumpkin.id);
        if (Helmet) {
...
async function equipBestArmor(bot) {
    await equipBestToolOrArmor(bot, "helmet");
    await equipBestToolOrArmor(bot, "chestplate");
    await equipBestToolOrArmor(bot, "leggings");
    await equipBestToolOrArmor(bot, "boots");
}